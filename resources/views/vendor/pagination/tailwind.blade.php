@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
                <p class="text-sm base-content leading-5">
                    {!! __('Mostrando') !!}
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        ...
                                        {{-- <span class="join-item btn btn-square">{{ $page }}</span> --}}
                                        <input class="join-item btn btn-square disabled" type="radio" name="options" aria-label="{{ $page }}" checked />
                                    </span>
                                @else
                                    <a href="{{ $url }}" class="join-item btn btn-square" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
